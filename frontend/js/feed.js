...
if (!postsEl || !myPostsEl) {
  ...
  return;
      ...
      const res = await fetch(API_BASE+"/api/posts/all", { headers: authHeader });
...
function attachShareHandlers() {
  ...
  document.querySelectorAll(".share-btn").forEach(btn => {
    ...
    newBtn.addEventListener("click", async (e) => {
      ...
      try {
        const fr = await fetch(API_BASE +"/api/friends/list", { headers: authHeader });
        ...
        friends.forEach(f => {
          ...
          shareBtn.addEventListener("click", async () => {
            ...
            try {
              const res = await fetch(API_BASE +"/api/posts/share", {
                method: "POST",
                headers: authHeader,
  ...
  function attachLikeHandlers() {
    document.querySelectorAll(".like-btn").forEach(btn => {
      ...
      newBtn.addEventListener("click", async () => {
        ...
        try {
          const res = await fetch(API_BASE +`/api/posts/${isUnlike ? "unlike" : "like"}`, {
            method: "POST",
            headers: authHeader,
            body: JSON.stringify({ postId }),
          });
...
function attachCommentHandlers() {
  ...
  document.querySelectorAll(".comment-send-btn").forEach(btn => {
    ...
    newBtn.addEventListener("click", async () => {
      ...
      if (!text) return;
      try {
        const res = await fetch(API_BASE +"/api/posts/comment", {
          method: "POST",
          headers: authHeader,
...
function initPostCreation() {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (file) {
        ...
        res = await fetch(API_BASE +"/api/posts/create", {
          method: "POST",
 ...
 function initFriendRequestSend() {
  ...
  if (!btn) return;
  btn.addEventListener("click", async () => {
    const nameInput = el("friend-email");
    const msgEl = el("friend-request-message");
    const name = nameInput?.value?.trim();
    if (!name) {
      if (msgEl) msgEl.textContent = "Enter a name.";
      return;
    }

    if (msgEl) msgEl.textContent = "Searching user...";
    try {
      ...
      if (msgEl) msgEl.textContent = data.message || "Friend request sent";
      if (nameInput) nameInput.value = "";

      // Optionally refresh lists
      await loadIncomingRequests();
      await loadFriends();
      await loadOtherUsers(); // keep list fresh
    } catch (err) {
...
async function loadFriends() {
  try {
    const res = await fetch(API_BASE +"/api/friends/list", { headers: authHeader });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
...
async function loadIncomingRequests() {
  try {
    const res = await fetch(API_BASE +"/api/friends/requests", { headers: authHeader });
    const data = await res.json().catch(() => ({}));
...
async function acceptRequest(requestId) {
  ...
  async function loadPostLimit() {
    try {
      const res = await fetch(API_BASE +"/api/posts/limit", { headers: authHeader });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
